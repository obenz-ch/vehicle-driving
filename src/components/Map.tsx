...
export const Map: React.FC<MapProps> = ({ vehicles, selectedVehicle, onVehicleSelect }) => {
  ...
  // Initialize Leaflet library and map
  useEffect(() => {
    const initializeMap = async () => {
      if (!mapContainerRef.current || mapRef.current || leafletLib) return;
  ...
  useEffect(() => {
    if (!mapRef.current || !leafletLib || !vehicles) return;
    ...
    const updateMarkers = () => {
      ...
      vehicles.forEach(vehicle => {
        ...
        const marker = leafletLib.marker([vehicle.gpsCoordinates.latitude, vehicle.gpsCoordinates.longitude], { icon })
          .addTo(mapRef.current!)
          .bindPopup(`
            <div class="p-2">
    ...
    updateMarkers();
  }, [vehicles, selectedVehicle, onVehicleSelect, leafletLib]);
